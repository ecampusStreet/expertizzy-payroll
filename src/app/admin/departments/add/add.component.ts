...
export class AddDepartmentComponent implements OnInit {
  ...
  prepareForm(){
    this.headOfTheDepartment = this.data && this.data.departmentHead ? this.data.departmentHead :'';
    this.department = new FormGroup({
      departmentName: new FormControl(this.data && this.data.departmentName? this.data.departmentName :'', [Validators.required]),
      departmentHead: new FormControl(this.data && this.data.departmentHead ? this.data.departmentHead :'', [Validators.required]),
      location: new FormControl(this.data && this.data.location? this.data.location :'', [Validators.required])
    });
    if(this.isReadOnly){
  ...
  submit(){
    ...
    const config={
      url: this.id ? urls.departments.update:urls.departments.create,
  ...
  setValue(){
    ...
    this.utils.dataFilter(config).then(resp =>{
      if(resp){
        this.headOfTheDepartment = resp;
        this.department.patchValue({
          departmentHead: resp.fullName
        });
      }
    })
  }
